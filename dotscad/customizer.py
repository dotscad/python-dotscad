...
class Customizer(OpenSCAD):
    ...
    __slots__ = ("vars", "tabs", "_customizer_source")
    ...
    def _load(self):
        ...
        parts = re.split(
            re.compile(r"(\n\s*module\s.+)$", re.DOTALL | re.IGNORECASE),
            self._source,
            2,
        )
        ...
        chunks = re.split(
            r"^(\s*/\*\s*\[\s*(.+?)\s*\]\s*\*/\s*)$", customizer_src, flags=re.MULTILINE
        )
        if len(chunks) == 1:
    ...
    def _tokenize_tab_vars(self, tab_name, source):
        ...
        for i in xrange(0, len(chunks), 5):
            ...
            self.vars[var_name] = CustomizerVar(
                var_name, decode_value(val), possible, desc
            )
            self.tabs[tab_name].append(self.vars[var_name])
            # Build the tokenized data
            tokenized += "// {0}\n{1} = ~~{1}~~; // [{2}]{3}".format(
                desc, var_name, possible, extra_scad
            )
        return tokenized
...
class CustomizerVar(object):
    ...
    __slots__ = ("name", "value", "possible", "description")
